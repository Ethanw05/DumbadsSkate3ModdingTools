...
public static class RenderMaterialDataBuilder
{
    ...
    /// <summary>
    /// Default attributor stream when none is provided (e.g. StartPark). "environment.default" is the
    /// full static-mesh shader (diffuse/lightmap/specular/normal/detail/macrooverlay) used by SkateSchool-style meshes.
    /// </summary>
    public const string DefaultAttributorStream = "environment.default";

    /// <summary>
    /// Attributor material that gets the forced micro-detail macro-overlay.
    /// </summary>
    private const string EnvironmentDefaultAttributor = "environment.default";

    /// <summary>
    /// Logical GUID of the standing micro-detail macro-overlay texture that ships in DONOTREMOVE
    /// (PS3: cpres/70DE2DE0FFA4F628.psg, X360: cpres_xbox/70D72AE0FF9ED2BA.rx2). It is force-bound to the
    /// macrooverlay channel of every <see cref="EnvironmentDefaultAttributor"/> material; authors cannot
    /// override the macrooverlay. The texture PSG/RX2 is merged into cPres_Global by DistPackRunner, so the
    /// GUID always resolves at runtime.
    /// </summary>
    public const ulong MacroOverlayGuidPs3 = 0x70DE2DE0FFA4F628UL;
    public const ulong MacroOverlayGuidX360 = 0x70D72AE0FF9ED2BAUL;

    // Stock-default macro-overlay scalars, mirroring BlenRose defaults (opacity 1.0, UV scale 0.3).
    // Applied alongside the forced overlay so the no-config default material renders it correctly.
    private const float ForcedMacroOverlayOpacity = 1.0f;
    private const float ForcedMacroOverlayUVScale = 0.3f;

    // environment.default's detail channel uses the default white texture (no authored detail map).
    // detailNormalUVScale mirrors BlenRose's default (8.0).
    private const float ForcedDetailNormalUVScale = 8.0f;

    /// <summary>
    /// Returns the platform-specific macro-overlay GUID to force when the material's attributor is
    /// <see cref="EnvironmentDefaultAttributor"/>; null otherwise (channel keeps its normal behavior).
    /// </summary>
    private static ulong? ForcedMacroOverlayGuid(string effectiveAttributor, ArenaPlatform platform)
        => string.Equals(effectiveAttributor, EnvironmentDefaultAttributor, StringComparison.OrdinalIgnoreCase)
            ? (platform == ArenaPlatform.Xbox360 ? MacroOverlayGuidX360 : MacroOverlayGuidPs3)
            : null;
        ...
        ArenaPlatform platform = ArenaPlatform.Ps3)
    {
        if (channelConfig != null)
            return BuildFromChannelConfig(materialName, overrides, nameChannelGuidOverride, attributorStream, channelConfig, platform);

        var channels = new List<ChannelDef>();
        var stringList = new List<string>();
        BuildGameCompatibleChannels(materialName, overrides, nameChannelGuidOverride, attributorStream, channels, stringList, platform);
        ...
        ArenaPlatform platform = ArenaPlatform.Ps3)
    {
        ...
        if (numMaterials == 1)
            return BuildGameCompatible(materialName, overrides, nameChannelGuidOverride, attributorStream, null, platform);

        var singleChannels = new List<ChannelDef>();
        var stringList = new List<string>();
        BuildGameCompatibleChannels(materialName, overrides, nameChannelGuidOverride, attributorStream, singleChannels, stringList, platform);
        ...
        ArenaPlatform platform = ArenaPlatform.Ps3)
    {
        ...
        {
            var p = parts[0];
            return BuildGameCompatible(p.MaterialName, p.Overrides, p.Overrides?.NameChannelGuid, p.AttributorStream, p.ChannelConfig, platform);
        }
        ...
        for (int i = 0; i < parts.Count; i++)
        {
            ...
            if (channelConfig != null)
                BuildFromChannelConfigChannels(materialName, overrides, overrides?.NameChannelGuid, attributorStream, channelConfig, channelsOut, stringListOut, platform);
            else
                BuildGameCompatibleChannels(materialName, overrides, overrides?.NameChannelGuid, attributorStream, channelsOut, stringListOut, platform);
        ...
        ArenaPlatform platform)
    {
        var channelsOut = new List<ChannelDef>();
        var stringListOut = new List<string>();
        BuildFromChannelConfigChannels(materialName, overrides, nameChannelGuidOverride, attributorStream, channelConfig, channelsOut, stringListOut, platform);
        ...
        ArenaPlatform platform)
    {
        ...
        // environment.default: force the standing micro-detail overlay onto the macrooverlay channel.
        // The overlay GUID is platform-specific and authors cannot override it.
        ulong? forcedMacroOverlayGuid = ForcedMacroOverlayGuid(effectiveAttributor, platform);
        string? macroOverlayStream = forcedMacroOverlayGuid.HasValue ? $"tex_0x{forcedMacroOverlayGuid.Value:x16}" : null;

        // Copy the authored channel list; ensure macrooverlay is present when the overlay is forced.
        var textureChannelNames = new List<string>(channelConfig.TextureChannelNames);
        if (forcedMacroOverlayGuid.HasValue &&
            !textureChannelNames.Any(n => string.Equals(n, "macrooverlay", StringComparison.OrdinalIgnoreCase)))
        {
            textureChannelNames.Add("macrooverlay");
        }

        var stringList = new List<string> { "Name", "AttribulatorMaterialName", effectiveAttributor, materialName };
        foreach (var texName in textureChannelNames)
        ...
        if (macroOverlayStream != null && !stringList.Contains(macroOverlayStream)) stringList.Add(macroOverlayStream);
        ...
        channelsOut.Clear();
        channelsOut.Add(new ChannelDef(offsets["Name"], (ushort)KNone, PegasusRwConstants.DefaultChannelMask, nameGuid, offsets[materialName], null));
        channelsOut.Add(new ChannelDef(offsets["AttribulatorMaterialName"], (ushort)KNone, PegasusRwConstants.DefaultChannelMask, attribulatorGuid, offsets[effectiveAttributor], null));

        foreach (var texName in textureChannelNames)
        {
            string texKey = texName.ToLowerInvariant();
            ulong guid = texKey switch
            {
                "diffuse" => diffuseGuid,
                ...
                "normal" => normalGuid,
                "noise" => noiseGuid,
                "macrooverlay" when forcedMacroOverlayGuid.HasValue => forcedMacroOverlayGuid.Value,
                _ => defaultWhiteGuid
            };
            string stream = texKey switch
            {
                ...
                "normal" => normalStream,
                "noise" => noiseStream,
                "macrooverlay" when macroOverlayStream != null => macroOverlayStream,
                _ => defaultWhiteStream
            };
            channelsOut.Add(new ChannelDef(offsets[texName], (ushort)KNone, PegasusRwConstants.DefaultChannelMask, guid, offsets[stream], null));
        }

        foreach (var (scalarName, val) in channelConfig.ScalarValues)
        {
            var constants = new float[4] { val, 0, 0, 0 };
            channelsOut.Add(new ChannelDef(offsets[scalarName], (ushort)KScalarConstant, PegasusRwConstants.DefaultChannelMask, 0, 0, constants));
        }
        ...
        ArenaPlatform platform)
    {
        ...
        // environment.default forces the standing micro-detail overlay on the macrooverlay channel
        // (platform-specific GUID). Mirror BlenRose's environment.default by also emitting the macro
        // opacity/UV-scale scalars so the overlay renders on no-config default materials.
        ulong? forcedMacroOverlayGuid = ForcedMacroOverlayGuid(effectiveAttributor, platform);
        string? macroOverlayStream = forcedMacroOverlayGuid.HasValue ? $"tex_0x{forcedMacroOverlayGuid.Value:x16}" : null;
        ...
        if (forcedMacroOverlayGuid.HasValue)
        {
            // environment.default full channel set: detail (white default) + macrooverlay (forced overlay) + 3 scalars.
            stringList.Add("detail");
            stringList.Add("detailNormalUVScale");
            stringList.Add("macrooverlay");
            if (macroOverlayStream != null && !stringList.Contains(macroOverlayStream)) stringList.Add(macroOverlayStream);
            stringList.Add("macroOverlayOpacity");
            stringList.Add("macroOverlayUVScale");
        }
        ...
        channelsOut.Add(new ChannelDef(offsets["Name"], (ushort)KNone, PegasusRwConstants.DefaultChannelMask, nameGuid, offsets[materialName], null));
        channelsOut.Add(new ChannelDef(offsets["AttribulatorMaterialName"], (ushort)KNone, PegasusRwConstants.DefaultChannelMask, attribulatorGuid, offsets[effectiveAttributor], null));
        channelsOut.Add(new ChannelDef(offsets["lightmap"], (ushort)KNone, PegasusRwConstants.DefaultChannelMask, lightmapGuid, offsets[lightmapStream], null));
        channelsOut.Add(new ChannelDef(offsets["specular"], (ushort)KNone, PegasusRwConstants.DefaultChannelMask, specularGuid, offsets[specularStream], null));
        channelsOut.Add(new ChannelDef(offsets["diffuse"], (ushort)KNone, PegasusRwConstants.DefaultChannelMask, diffuseGuid, offsets[diffuseStream], null));
        channelsOut.Add(new ChannelDef(offsets["normal"], (ushort)KNone, PegasusRwConstants.DefaultChannelMask, normalGuid, offsets[normalStream], null));
        if (forcedMacroOverlayGuid.HasValue && macroOverlayStream != null)
        {
            channelsOut.Add(new ChannelDef(offsets["detail"], (ushort)KNone, PegasusRwConstants.DefaultChannelMask, defaultWhiteGuid, offsets[defaultWhiteStream], null));
            channelsOut.Add(new ChannelDef(offsets["detailNormalUVScale"], (ushort)KScalarConstant, PegasusRwConstants.DefaultChannelMask, 0, 0, new float[4] { ForcedDetailNormalUVScale, 0, 0, 0 }));
            channelsOut.Add(new ChannelDef(offsets["macrooverlay"], (ushort)KNone, PegasusRwConstants.DefaultChannelMask, forcedMacroOverlayGuid.Value, offsets[macroOverlayStream], null));
            channelsOut.Add(new ChannelDef(offsets["macroOverlayOpacity"], (ushort)KScalarConstant, PegasusRwConstants.DefaultChannelMask, 0, 0, new float[4] { ForcedMacroOverlayOpacity, 0, 0, 0 }));
            channelsOut.Add(new ChannelDef(offsets["macroOverlayUVScale"], (ushort)KScalarConstant, PegasusRwConstants.DefaultChannelMask, 0, 0, new float[4] { ForcedMacroOverlayUVScale, 0, 0, 0 }));
        }
